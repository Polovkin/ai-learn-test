...
export const calculateCalories = ({ weightKg, distanceKm }: CalculateCaloriesArgs): CalculateCaloriesResult => {
  const calories = Math.round(weightKg * distanceKm * 1.036)
  return { calories }
}

export const calculateTravelTime = ({ distanceKm, speedKmH }: CalculateTravelTimeArgs): CalculateTravelTimeResult => {
